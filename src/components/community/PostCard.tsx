...
export function PostCard({ post, index, onModerationAction }: PostCardProps) {
  const { t } = useTranslation();
  const tr = (value: string) => t(value, { defaultValue: value });
    ...
    >
      ...
      <div className="relative p-5">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <div className="relative" ref={menuRef}>
              <button
                ...
                title={tr("Moderation")}
                aria-label={tr("Moderation")}
              >
                <MoreHorizontal className="w-4 h-4" />
              </button>
              {moderationOpen && (
                <div className="absolute right-0 mt-1 w-40 rounded-xl border border-gray-200/70 dark:border-gray-800/80 bg-white/95 dark:bg-gray-900/95 shadow-lg z-20 overflow-hidden">
                  <button
                    type="button"
                    onClick={() => {
                      onModerationAction?.("report", post);
                      setModerationOpen(false);
                    }}
                    className="w-full px-3 py-2 text-left text-xs font-semibold text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
                  >
                    {tr("Report Post")}
                  </button>
                  <button
        ...
        {(post.pgn || post.fen) && !post.gameResult && (
          <MiniBoardPreview label={tr("View Position")} />
        )}
        {post.pgn && post.gameResult && (
          <MiniBoardPreview label={tr("Replay Game")} />
        )}
